---
title: "A Solution to the Price Puzzle on US Data"
author:
  - name: Florian Jacquetin
  - name: Loman Sezestre
date: today
format:
  html:
...
1.  Data
2.  Price Puzzle in the NK framework
3.  Price Puzzle in US data
4.  Solving the Price Puzzle: pre-Volcker regime
5.  Extensions to post-Volcker regimes
...
We start with a model with 3 variables.
...
We run another Chow test to find a break point during the 2008 crisis.
...
# Price Puzzle in the NK framework

Here we simulate IRFs from a theoretical NK model.
...
Based on this NK model, we simulate artificial but economically realistic macroeconomic time series and then estimate a VAR on these simulated data.
...
# Price Puzzle on US data

We construct a SVAR model based on real US data \[og(t) pi(t) ffr(t)\]
...
df_irf_pre$period  <- factor("Pré-1979", levels = c("Pré-1979", "Post-1979","Post-2009"))
df_irf_post$period <- factor("Post-1979", levels = c("Pré-1979", "Post-1979","Post-2009"))
df_irf_end$period <- factor("Post-2009", levels = c("Pré-1979", "Post-1979","Post-2009"))

nk_df_long <- nk_df %>%
  mutate(lower = irf, upper = irf) %>%
  expand_grid(period = c("Pré-1979", "Post-1979","Post-2009"))

df_irf_all <- bind_rows(df_irf_pre, df_irf_post,df_irf_end)


df_irf_all_plot <- df_irf_all %>%
  mutate(period_factor = factor(period, levels = c("Pré-1979", "Post-1979","Post-2009")))
...
ggplot() +
  ...
  scale_color_manual(values = c("Pré-1979" = "red", "Post-1979" = "blue","Post-2009" = "green")) +
  scale_fill_manual(values = c("Pré-1979" = "red", "Post-1979" = "blue", "Post-2009" = "green")) +
...
We want to illustrate where the price puzzle comes from before 1979 and after 2009. We plot the Forecast Error Variance Decomposition (FEVD) after the ffr shock.
...
fevd_pi_all <- bind_rows(
  ...
  mutate(
    period = factor(period, levels = c("Pré-1979", "1979-2006", "Post-2009"))
  )
...
Let's see the estimated coefficients of the VAR.
...
Before 1979, the ffr coefficient is positive and large. Before Volcker, the Fed typically raised interest rates in response to expected increases in inflation (poor monetary policy rules, focus on output and unemployment, weak anti-inflation credibility). The VAR confuses cause and consequence. No Phillips curve effect : output gap has no effect on inflation. Apparent but spurious price puzzle.

After 1979, the Fed aggressively fights inflation (Volcker disinflation). Breakdown of the standard VAR structure. Coefficients on interest rates are huge and unstable. It is typical of the ZLB / QE environment.

# Solving the price puzzle: pre-Volcker regime

Following Castelnuovo et al., we propose a sign restrictions approach. We impose that after a monetary shock, ffr \> 0 and outputggap \< 0. Inflation remains unconstrained.
...
We observe the IRFs at t=0 in the plane (ffr(0), ogap(0))
...
The puzzle still exists but is less apparent. Let's see the coefficients.
...
While the sensitivity of inflation to the federal funds rate becomes negative, a monetary policy shock still leads to an increase in inflation expectations in the short run. This pattern can be explained either by a **central bank information effect**, whereby policy tightening signals higher expected inflation to markets, or by **limited short-run credibility of the Federal Reserve** in anchoring inflation expectations.

Let's compare inflation and inflation forecasts.
...
Now let's include sign restrictions in the VAR with 4 variables.
...
This combination of sign restrictions and forecast variables does not outperform either identification strategy taken separately. In the VAR, this reflects the fact that once central bank information is explicitly controlled for, the identified monetary policy shock no longer corresponds to a purely exogenous policy disturbance. In particular, during the pre-1979 period, monetary policy appears to have been largely endogenous, reacting primarily to agents’ inflation expectations rather than exerting an independent stabilizing role.
...
# Extensions to post-Volcker regimes

We apply these strategies to post-Volcker regime. It's not relevant, and then counterproductive with the Greenbook. Post-Volcker, inflation forecasts are an integral part of the monetary policy transmission mechanism. Conditioning on them removes the credibility channel rather than isolating an information effect, leading to a re-emergence of the price puzzle.
...
The estimated reduced-form VAR reveals that inflation expectations respond positively to lagged policy rates, while inflation itself is mainly driven by its own persistence and expectations. Monetary policy reacts strongly to past inflation and expectations, implying that policy innovations are largely endogenous. As a result, conditioning on inflation forecasts removes part of the credibility channel rather than isolating an exogenous monetary shock

```{r}
lapply(var_post_gb$varresult, function(eq) coef(eq))
```

The price puzzle reappears in the 2010s, a period characterized by policy rates close to the zero lower bound, where rate increases primarily convey information rather than exert immediate contractionary effects.
...
Let's see the post-2008 coefficients. When the policy rate is close to zero, the VAR suggests that movements in the federal funds rate primarily reflect signaling normalization effects and increases expectations, leading to a price puzzle.

```{r}
lapply(var_end_gb$varresult, function(eq) coef(eq))
```
