...
# 1. Charger la série mensuelle WTI -> trimestrielle log-diff
oil_m <- fredr::fredr(
  series_id = "WTISPLC",
  observation_start = as.Date("1960-01-01")
) %>%
...
# 2. Fusion avec données US (y_gap, pi, ffr)
df_us_oil <- df_us %>%
  inner_join(oil_q, by = "date") %>%
  arrange(date)

# 3. Définir les 3 sous-périodes
...
Y_pre_ts_oil  <- make_ts4(df_pre_oil)
Y_post_ts_oil <- make_ts4(df_post_oil)
Y_end_ts_oil  <- make_ts4(df_end_oil)
